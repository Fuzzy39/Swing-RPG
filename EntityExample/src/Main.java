...
public class Main 
{
	...
	public static void main(String[] args) 
	{
		...
		goblin = Engine.loadTexture("goblin.jpg");
		...
		engine.getMainUI().setStatusText("Get the 10 coins! Avoid the monsters!");
	...
	private static World createWorld(Engine engine)
	{
		...
		TileType q = new TileType(o);
		q.setOnEnter((Tile t)->{engine.getMainUI().resetText();});
		...
		EntityType monster = new EntityType(goblin, new Point(50,50));
		monster.setUpdatePriority(UpdatePriority.Always);
		monster.setFriction(0);
		monster.setOnCollide((Entity e, Direction d)->
		{
			Point newVel = new Point(e.getVelocity());
			switch(d)
			{
				case UP:
				case DOWN:
		...
		//monster.setCollidesWithWalls(false);
		...
		world.initialize(new Point(50,50), player, screen);
		
		// terrible code
		new Entity(world.getCurrentScreen(), new Point(400, 50),monster  ).setVelocity(new Point(-8, 8));
		
		new Entity(world.getCurrentScreen(), new Point(500, 300),monster  ).setVelocity(new Point(8, -8));
		
		new Entity(world.getCurrentScreen(), new Point(500, 250),monster  ).setVelocity(new Point(-7, 7));
		
		//new Entity(world.getCurrentScreen(), new Point(50, 500),monster  ).setVelocity(new Point(7, -7));
		
		new Entity(world.getCurrentScreen(), new Point(400, 500),monster  ).setVelocity(new Point(-7, 7));
		
		new Entity(world.getCurrentScreen(), new Point(650, 700),monster  ).setVelocity(new Point(6, -6));
		
		
		player.setOnUpdate((Entity playerE)->{
			for(Entity ent : world.getEntities())
			{
				...
				if(ent.getBounds().intersects(playerE.getBounds()))
				{
					deaths++;

					playerE.teleportGlobal(50, 50);
					engine.getMainUI().setText("You died! (total: "+deaths+")");
	...
	private static TileType configureCoin(Engine engine, TileType floor)
	{
		...
		c.setOnEnter((Tile t)->
		{
			...
			UI.setOnInteract(()->
			{
				...
				UI.setStatusText("Only "+coinsLeft+" left. Avoid the Monsters!" );
				...
				if(coinsLeft == 0)
				{
					UI.setStatusText("You win! You died "+deaths+" times.");
				}
